(function () {
        ...
        request.onreadystatechange = function () {
            if (request.readyState === XMLHttpRequest.DONE) {
                if (request.status === 200) {
                    window.cardResponseTime = performance.now();                    
    ...
    // force to call API with a new card in order to test api response time with lihgtHouse 
    var key = 'metros/1/champs+elysees+clemenceau/A';
    var label= 'Champs-Elysees-Clemenceau, Direction La Défense';
    app.getSchedule(key, label);
    app.selectedTimetables.push({key: key, label: label});
    app.saveSelectedTimetables();
